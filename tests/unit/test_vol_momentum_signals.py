...
def decide(momentum, *, position=0, observations=42, ready=True, exists=True, return_diagnostics=False):
    # About 60 calendar days contains 42 daily bars, never 61 bars. Alternating
    # 1% daily moves have about 16% annual volatility / 24% signal threshold.
    ...
    context = SimpleNamespace(
        ...
        Market={"AAPL": SimpleNamespace(Exists=exists, Close=prices[-1], History=lambda window: history)},
        Portfolio=SimpleNamespace(cash=100000, total_value=100000,
...
@pytest.mark.parametrize("momentum,position,bullish,bearish,buy,sell", [
    (50, 0, 1, 0, 1, 0),
    (1, 0, 0, 0, 0, 0),
    (-50, 10, 0, 1, 0, 1),
    (-50, 0, 0, 1, 0, 0),
])
def test_diagnostics_distinguish_thresholds_from_trade_requests(momentum, position, bullish, bearish, buy, sell):
    ...
    assert 23 < snapshot["thresholdPct"] < 25
    assert snapshot["momentumToThresholdRatio"] == pytest.approx(momentum / snapshot["thresholdPct"])
...
@pytest.mark.parametrize("momentum,kwargs,explanation", [
...
def test_zero_fill_summary_explains_warmup_signals_and_execution(momentum, kwargs, explanation):
    ...
    assert "Trade metrics that require executed or closed trades are unavailable." in summary["message"]
...
def test_fast_summary_does_not_infer_no_trades_from_an_empty_fill_table():
    ...
    assert _execution_summary("event", [{"ticker": "AAPL"}], None) == {"fillCount": 1, "message": None}
